...
# MDD_test1<-read.table("C:/Users/sunshiny/Downloads/SLEMDD/MDD/test/GSE39653_series_matrix.txt.gz"
#                      ,sep="\t",header=T,row.names = 1,comment.char='!')
# MDD_test2<-read.table("C:/Users/sunshiny/Downloads/SLEMDD/MDD/test/GSE52790_series_matrix.txt.gz"
#                       ,sep="\t",header=T,row.names = 1,comment.char='!')
# library(data.table)
# meta2<-fread("C:/Users/sunshiny/Downloads/SLEMDD/MDD/test/GSE52790_meta.txt",data.table=F,header=F)
...
# 
# meta1<-fread("C:/Users/sunshiny/Downloads/SLEMDD/MDD/test/GSE39653_meta.txt",data.table=F,header=F)
# colnames(meta1)<-c("name","case")
# meta1$case<-substr(meta1$case,1,2)
# table(meta1$case)
# meta1<-subset(meta1,!meta1$case=="BD")
# table(meta1$case)
# meta1$group<-ifelse(meta1$case == "HC", 0, 1)
# table(meta1$group)
# MDD_test1<-MDD_test1[,meta1$name]
...
#############GSE289146
#受压力相关精神疾病影响的个体 （n=210,81% 有抑郁症状） 和未受影响 （n=95） 的个体PBMC



# #############GSE39653做出来的结果非常差，不要
# library(GEOquery)
# library(limma)
# library(affy)
# library(data.table)
# gset <- getGEO('GSE39653', destdir="./",
#                AnnotGPL = T,     ## 注释文件
#                getGPL = T)       ## 平台文件
# gset[[1]]
# exp<-exprs(gset[[1]])
# ## 获取临床信息
# cli<-pData(gset[[1]])
# meta1<-data.frame(cli$geo_accession,cli$title)
# colnames(meta1)<-c("name","case")
# meta1$case<-substr(meta1$case,1,2)
# table(meta1$case)
# meta1<-subset(meta1,!meta1$case=="BD")
...
# table(duplicated(exp_symbol$`Gene symbol`))
# exp_unique<-avereps(exp_symbol[,-c(1,ncol(exp_symbol))],ID=exp_symbol$`Gene symbol`)
# MDD_test1<-exp_unique[,meta1$name]
# write.csv(MDD_test1, file = "meta1_expr.csv", row.names = T,quote = F)
# write.csv(meta1, file = "meta1.csv", row.names = T,quote = F)

##################################	合并
setwd("C:/Users/sunshiny/Downloads/SLEMDD/MDD/test/")
MDD1<-read.csv("GSE289146/meta1_expr_log.csv",row.names = 1)
MDD2<-read.csv("meta2_expr.csv",row.names = 1)
MDD1_meta<-read.csv("GSE289146/meta1.csv",row.names = 1)
MDD2_meta<-read.csv("meta2.csv",row.names = 1)
...
MDD<-merge(MDD1,MDD2,by="row.names")
rownames(MDD)<-MDD[,1]
MDD<-MDD[,-1]
MDD[is.na(MDD)]<-0
summary(colSums(MDD))
...
# normalized
# normalized_matrix <- apply(MDD, 2, function(x) {
#   x * (100000 / sum(x))
# })
# summary(colSums(normalized_matrix))
library(sva)
library(ggplot2)
...
normalized_matrix<-MDD
exprSet = as.matrix(normalized_matrix)
head(exprSet)[1:4,1:4]
batch = MDD_meta$batch
head(batch)
# "GSE39653" "GSE39653" "GSE39653" "GSE39653" "GSE39653" "GSE39653"
...
ac <- data.frame(
  ...
  Case = as.character(MDD_meta$group))
# 设置图片标题
proj = "MDDbatch"
pro = proj
...
# batch<-gsub("GSE39653",1,batch)
# batch
# batch<-gsub("GSE52790",2,batch)
...
pheno$cancer <- factor(pheno$group, 
                       levels = c(0,1))
...
write.csv(expr_combat, file = "MDDtest_expr_combat.csv", row.names = T,quote = F)
write.csv(MDD_meta, file = "MDDtest_meta.csv", row.names = T,quote = F)
